...
color_train_dir = os.path.join(base_dir, "processed", "color", "train")
color_test_dir = os.path.join(base_dir, "processed", "color", "test")
ARCHITECTURE_NAME = 'cnn_color_layers_kernels'
RESULTS_DIR = '../result'
...
class CNN:
    def __init__(self, input_shape, num_classes, num_channels=3):
    ...
    def build_model(self):
        ...
        layers = [
            InputLayer(),
            ConvolutionalLayer(kernel_size=(3, 3), num_channels=C, num_kernels=16),
            ReLULayer(),
            MaxPoolLayer(pool_size=2, stride=2),
            ConvolutionalLayer(kernel_size=(3, 3), num_channels=16, num_kernels=32),
            ReLULayer(),
            MaxPoolLayer(pool_size=2, stride=2),
            FlatteningLayer(),
            FullyConnectedLayer(sizeIn=6 * 6 * 32, sizeOut=128),
            ReLULayer(),
            FullyConnectedLayer(sizeIn=128, sizeOut=self.num_classes),
            SoftmaxLayer(),
        ]
        return layers
    ...
    def train(self, X_train, y_train, X_test, y_test, epochs=50, batch_size=32, learning_rate=0.05):
        ...
        for epoch in range(epochs):
            ...
            print(f"Epoch {epoch + 1}/{epochs}, Train Loss: {train_loss:.4f}, Train Acc: {train_accuracy:.4f}, "
                  f"Test Loss: {test_loss:.4f}, Test Acc: {test_accuracy:.4f}, Time: {time.time() - start_time:.2f}s")
...
if __name__ == "__main__":
    ...
    cnn = CNN(input_shape=input_shape, num_classes=num_classes,num_channels=3)
    history = cnn.train(X_train, y_train, X_test, y_test, epochs=50, batch_size=32, learning_rate=0.01)
